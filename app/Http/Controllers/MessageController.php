        ...
        if(count($con)>0)
        {
            ...
            if(Auth::id() == $user_one)
            {
                ...
                if(count($mess)>0)
                {
                    ...
                    return response([$data,$ids,$date,$time,count($mess)==$request->numberofmessages?1:0]);
                }
...
//            }
            ...
            return view('Employer.ManageMessage.inbox')->with('msgdate',$msgdate)->with('msgtimes',$msgtimes)->with('msgids',$msgids)->with('unseen',$unseen)->with('total_message',$this->total_messages)
    ...
    public function messagebox($id)
    {
        ...
        return view('Employer.ManageMessage.sendmessageform')->with('id',$id)->with('shown',$this->shown)->with('total_message',$this->total_messages)
            ->with('data',$this->user_data)->with('time',$this->time)->with('ids',$this->ids)->with('user',$user);
    }
