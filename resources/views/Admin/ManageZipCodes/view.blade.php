        ...
        <div class="page-wrapper" style="min-height: 542px;">
            ...
            <div class="row page-titles">
                <div class="align-self-center">
                    <h3 class="text-primary text-center">View Zip</h3> </div>
                                    ...
                                    <table class="table table-hover ">
                                        ...
                                        @foreach($zip_code as $z)
                                            <tr>
                                                    ...
                                                    <th>{{ $z->city->state->country->country_name  }}</th>
                                                    <th>{{ $z->city->state->state_name  }}</th>
                                                    <th>{{ $z->city->city_name  }}</th>
                                                    <th>{{ $z->zip_code  }}</th>
                                                <th>
                                                <div class="button-container">
                                                        <form action="{{ route('ZipCodes.edit',$z->id) }}" method="get">
                                                            <div>
                                                                <button type="submit" class="btn btn-primary"><span class="glyphicon glyphicon-edit" title="Edit"></span></button>
                                                            </div>
                                                        </form>
                                                        <form action="{{ route('ZipCodes.destroy' ,$z->id) }}" method="POST">
                                                            <div>
                                                                {{ method_field('DELETE') }}
                                                                {{csrf_field()}}
